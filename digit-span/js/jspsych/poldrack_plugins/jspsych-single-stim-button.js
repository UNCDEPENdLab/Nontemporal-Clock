...
jsPsych.plugins["single-stim-button"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
	...
	$('.' + trial.button_class).on('click',function(){
		// display_element.html(response.mouse);
		if(response.mouse == -1){
			...
			if ($(this).attr('id') != undefined) {
				...
				// display_element.html(response.mouse);
				// return '<div class = centerbox><div class = center-text>' + response.mouse + '</p></div>'
				// display_element.html(response);
			} else {
				response.mouse = 'undefined_button'
			}
			// display_element.append(response.mouse);
