function [nx, ny, textbounds] = DrawFormattedTextBounds(win, tstring, sx, sy, color, wrapat, flipHorizontal, flipVertical, vSpacing, righttoleft, winRect)
%The following code is from PsychToolbox, v 3.0.11, written and modified by Mario Kleiner
%It was modified by Brad Wyble on June 2015 so that it calculates text
%bounds without actually drawing the stimuli on the screen

% [nx, ny, textbounds] = DrawFormattedTextBound(win, tstring [, sx][, sy][, color][, wrapat][, flipHorizontal][, flipVertical][, vSpacing][, righttoleft][, winRect])
%
  ...
  lastyp = yp;
    ...
    if ~isempty(curstring) && noclip
        ...
        if flipHorizontal || flipVertical
            ...
            Screen('glTranslate', win, -xc, -yc, 0);
            % commented out by BPW June2015
            
%            Screen('TextBounds', Parameters.window, stimuli{stim});
            
           boundsrect = Screen('TextBounds', win, curstring);
    ...
    if dolinefeed
        ...
        %Edited by Brad Wyble June 2015
